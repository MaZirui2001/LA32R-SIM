...
void statistic::stall_update(bool _stall_by_fetch_queue, bool _stall_by_rename, bool _stall_by_rob, bool _stall_by_iq1, bool _stall_by_iq2, bool _stall_by_iq3, bool _stall_by_iq4, bool _stall_by_sb){
...
void statistic::print_stat(){
    ...
    Log("Stall by fetch queue = %.3lf%%, Stall by rename = %.3lf%%, Stall by rob = %.3lf%%, Stall by iq1 = %.3lf%%, Stall by iq2 = %.3lf%%, Stall by iq3 = %.3lf%%, Stall by iq4 = %.3lf%%, Stall by sb = %.3lf%%", 
        double(stall_by_fetch_queue) / total_clocks * 100, double(stall_by_rename) / total_clocks * 100, double(stall_by_rob) / total_clocks * 100, double(stall_by_iq1) / total_clocks * 100, double(stall_by_iq2) / total_clocks * 100, double(stall_by_iq3) / total_clocks * 100, double(stall_by_iq4) / total_clocks * 100, double(stall_by_sb) / total_clocks * 100);
}
